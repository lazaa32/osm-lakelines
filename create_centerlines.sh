...
if [ ! -f $CENTERLINES_GEOJSON ]; then
	...
	if [ -f $FULL_PBF ]; then
		...
		query="SELECT osm_id, ST_makeValid(ST_SimplifyVW(geometry,(ST_Npoints(geometry) / 12000) * 100000)) AS geometry FROM osm_lake_polygon WHERE area > 2 * 1000 * 1000 AND ST_GeometryType(geometry)IN ('ST_Polygon','ST_MultiPolygon') AND name <> '' ORDER BY area DESC"
		...
		label_centerlines --verbose --max_points=12000 --simplification=0.01 --smooth=2 --max_paths=1 --output_driver GeoJSON "$LAKE_SHP" "$CENTERLINES_GEOJSON"
		#label_centerlines --verbose --max_points=12000 --simplification=0.01 --smooth=2 --max_paths=1 --output_driver GPKG "$LAKE_SHP" "$CENTERLINES_GPKG"
